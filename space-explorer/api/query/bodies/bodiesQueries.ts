...
const astronomyApiService = axios.create({
    ...
    auth: {
        username: process.env.EXPO_PUBLIC_ASTRONOMY_API_USERNAME!,
        password: process.env.EXPO_PUBLIC_ASTRONOMY_API_PASSWORD!
    },
});
...
export const useGetBodiesPositions = (latitude: number, longitude: number) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const currentTime = currentDateTime.substring(currentDateTime.indexOf("T") + 1);
